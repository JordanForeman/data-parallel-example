...
int main(int argc, const char * argv[])
{
    ...
    kernel = clCreateKernel(program, "matrixmult", NULL);
    ...
    errNum = clEnqueueNDRangeKernel(commandQueue, kernel, 1, NULL,workItems, groupSize,0, NULL, NULL);
    
    if (errNum != CL_SUCCESS)
        std::cout << "Could not queue the kernel\n";
    
    //Eighth Read the results in any results buffers that were changed
    errNum = clEnqueueReadBuffer(commandQueue, resultBuffer, CL_TRUE,0, ARRAY_SIZE * ARRAY_SIZE * sizeof(int), result,0, NULL, NULL);
